...
function childHandlers(root) {
    return isDir(root)
        ? root.children.filter(isFile).filter(e => isAllowedHandlerFile(e))
        : [];
}
...
function parseDir(root) {
    return {
        ...
        handlers: dictPartition(
            e => e.method,
            childHandlers(root).map(parseHandler)
        ),
...
function flattenBranch(prefix, branch, target) {
    ...
    if (hasHandlers(branch)) {
        Object.keys(branch.handlers).forEach(method => {
            // use map?
            target.push([method, herePath, branch.handlers[method]]);
        });
    }
...
module.exports.flattenRoutes = flattenRoutes;
