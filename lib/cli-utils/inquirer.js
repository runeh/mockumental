...
function routeFormatter(routes) {
    ...
    return route => {
        ...
        return `${ disabled ? '' : '- ' } ${ padTo(
            maxPathWidth,
            route.path
        ) } [${ padTo(maxMethodWidth, route.method) }] ${ handlerString }`;
    };
}
...
function handlerFormatter(handlers) {
    ...
    return (handler, selected) => {
        return `${ padTo(
            maxNameWidth,
            handler.prettyName
        ) } [${ handler.status }] [${ handler.type }] ${ selected ? '(active)' : '' }`;
    };
}
            ...
            choices: ({ route }) => {
                ...
                return route.handlers.map(handler => {
                    return {
                        name: formatter(
                            handler,
                            selectedHids.includes(handler.handlerId)
                        ),
                        value: handler,
